...
export default function Login() {
  ...
  const inputCls = "w-full bg-black/50 border border-white/15 text-white px-4 py-3 text-sm rounded-sm focus:outline-none focus:ring-1 focus:ring-[#FF5F15] placeholder:text-zinc-500 font-body";
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md animate-rise">
          ...
          <form onSubmit={submit} className="space-y-4">
            {mode === "register" && (
              <>
                <input data-testid="register-name-input" className={inputCls} placeholder="Full name" value={form.name} onChange={set("name")} required />
                <select data-testid="register-role-select" className={inputCls} value={form.role} onChange={set("role")}>
                  <option value="client">Client — submit requests</option>
                  <option value="reviewer">Reviewer — approve plans</option>
                </select>
              </>
            )}
            <input data-testid="login-email-input" className={inputCls} type="email" placeholder="Email address" value={form.email} onChange={set("email")} required />
            <input data-testid="login-password-input" className={inputCls} type="password" placeholder="Password" value={form.password} onChange={set("password")} required />
            <button data-testid="login-submit-button" disabled={busy}
              ...
              {busy ? "Authenticating…" : mode === "login" ? "Enter Control Center" : "Create Account"}
            </button>
          </form>
