...
function EngineSection({ job, generating, onGenerate }) {
...
function ReviewSection({ job, onJobUpdated }) {
...
function RequestDetails({ job }) {
  const rows = [["Works", job.works_type], ["Road", job.road_type], ["Lanes", `${job.lanes_closed}/${job.lanes_total} closed`], ["Speed", `${job.speed_limit} km/h`], ["Volume", job.traffic_volume], ["Duration", job.duration || "TBD"], ["Client", job.client_name]];
  return (
    ...
    <aside data-testid="actions-panel" className="w-80 flex-shrink-0 bg-[#0A0A0A] border-l border-white/10 h-full overflow-y-auto flex flex-col p-6 noise-overlay">
      ...
      {!job ? (
        ...
        <div className="space-y-7">
          ...
          <EngineSection job={job} generating={generating} onGenerate={onGenerate} />

          {job.plan && isReviewer && job.status !== "approved" && <ReviewSection job={job} onJobUpdated={onJobUpdated} />}

          {job.plan && (
            <div>
              <span className={LABEL}>Export</span>
              <button data-testid="export-pdf-button" onClick={() => exportPdf(job)}
